...
struct BoundingSphere {
    ...
    constexpr BoundingSphere(_In_ const XMFLOAT3& center, _In_ float radius) noexcept : Center(center), Radius(radius) {
    }
    ...
    ContainmentType XM_CALLCONV ContainedBy(_In_ FXMVECTOR Plane0, _In_ FXMVECTOR Plane1, _In_ FXMVECTOR Plane2, _In_ GXMVECTOR Plane3, _In_ HXMVECTOR Plane4, _In_ HXMVECTOR Plane5) const noexcept;
    ...
    static void CreateFromPoints(_Out_ BoundingSphere& Out, _In_ size_t Count, _In_reads_bytes_(sizeof(XMFLOAT3) + Stride * (Count - 1)) const XMFLOAT3* pPoints, _In_ size_t Stride) noexcept;
...
struct BoundingBox {
    ...
    constexpr BoundingBox(_In_ const XMFLOAT3& center, _In_ const XMFLOAT3& extents) noexcept : Center(center), Extents(extents) {
    }
    ...
    static void CreateFromPoints(_Out_ BoundingBox& Out, _In_ size_t Count, _In_reads_bytes_(sizeof(XMFLOAT3) + Stride * (Count - 1)) const XMFLOAT3* pPoints, _In_ size_t Stride) noexcept;
};
...
struct BoundingOrientedBox {
    ...
    constexpr BoundingOrientedBox(_In_ const XMFLOAT3& center, _In_ const XMFLOAT3& extents, _In_ const XMFLOAT4& orientation) noexcept : Center(center), Extents(extents), Orientation(orientation) {
    }
    ...
    static void CreateFromPoints(_Out_ BoundingOrientedBox& Out, _In_ size_t Count, _In_reads_bytes_(sizeof(XMFLOAT3) + Stride * (Count - 1)) const XMFLOAT3* pPoints, _In_ size_t Stride) noexcept;
};
...
struct BoundingFrustum {
    ...
    // Creators
    BoundingFrustum() noexcept : Origin(0, 0, 0), Orientation(0, 0, 0, 1.f), RightSlope(1.f), LeftSlope(-1.f), TopSlope(1.f), BottomSlope(-1.f), Near(0), Far(1.f) {
    }
