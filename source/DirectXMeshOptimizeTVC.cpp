...
class mesh_status {
   public:
    mesh_status() noexcept : mUnprocessed{}, mFaceOffset(0), mFaceCount(0), mMaxSubset(0), mTotalFaces(0) {
    }

    HRESULT initialize(_In_reads_(nFaces * 3) const index_t* indices, size_t nFaces, _In_reads_(nFaces * 3) const uint32_t* adjacency, const std::vector<std::pair<size_t, size_t>>& subsets) {
        ...
        for (const auto& it : subsets) {
            ...
            for (uint32_t face = faceOffset; face < faceMax; ++face) {
                ...
                if (i0 == index_t(-1) || i1 == index_t(-1) || i2 == index_t(-1) || i0 == i1 || i0 == i2 || i1 == i2) {
                    ...
                    for (uint32_t n = 0; n < 3; ++n) {
                        ...
                        if (neighbor != UNUSED32) {
                            if ((neighbor < faceOffset) || (neighbor >= faceMax) || (neighbor == adjacency[face * 3 + ((n + 1) % 3)]) || (neighbor == adjacency[face * 3 + ((n + 2) % 3)])) {
    ...
    HRESULT setSubset(_In_reads_(nFaces * 3) const index_t* indices, size_t nFaces, size_t faceOffset, size_t faceCount) noexcept {
   ...
   private:
    ...
    void remove(uint32_t faceIndex) noexcept {
        ...
        mListElements[faceIndex].prev = mListElements[faceIndex].next = UNUSED32;
    }
...
_Use_decl_annotations_ HRESULT DirectX::OptimizeFaces(const uint16_t* indices, size_t nFaces, const uint32_t* adjacency, uint32_t* faceRemap, uint32_t vertexCache, uint32_t restart) {
...
_Use_decl_annotations_ HRESULT DirectX::OptimizeFaces(const uint32_t* indices, size_t nFaces, const uint32_t* adjacency, uint32_t* faceRemap, uint32_t vertexCache, uint32_t restart) {
    if (!indices || !nFaces || !adjacency || !faceRemap)
        return E_INVALIDARG;
...
DirectX::OptimizeFacesEx(const uint16_t* indices, size_t nFaces, const uint32_t* adjacency, const uint32_t* attributes, uint32_t* faceRemap, uint32_t vertexCache, uint32_t restart) {
...
_Use_decl_annotations_ HRESULT
DirectX::OptimizeFacesEx(const uint32_t* indices, size_t nFaces, const uint32_t* adjacency, const uint32_t* attributes, uint32_t* faceRemap, uint32_t vertexCache, uint32_t restart) {
    if (!indices || !nFaces || !adjacency || !attributes || !faceRemap)
        return E_INVALIDARG;
