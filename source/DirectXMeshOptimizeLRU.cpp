...
template <typename IndexType>
HRESULT OptimizeFacesImpl(_In_reads_(indexCount) const IndexType* indexList, uint32_t indexCount, _Out_writes_(indexCount / 3) uint32_t* faceRemap, uint32_t lruCacheSize, uint32_t offset) {
    ...
    for (size_t i = 0; i < indexCount; i += 3) {
        if (vertexRemap[i] == UNUSED32 || vertexRemap[i + 1] == UNUSED32 || vertexRemap[i + 2] == UNUSED32) {
            ++bestFace;
            continue;
        } else
            break;
            ...
            while (nextBestFace < faceCount) {
                ...
                if (processedFaceList[faceIndex] == 0) {
                    ...
                    if (i0 != UNUSED32 && i1 != UNUSED32 && i2 != UNUSED32) {
                        ...
                        bestFace = face;
                        bestScore = vertexDataList[i0].score + vertexDataList[i1].score + vertexDataList[i2].score;
                        break;
                    }
                }
            }
...
_Use_decl_annotations_ HRESULT DirectX::OptimizeFacesLRU(const uint16_t* indices, size_t nFaces, uint32_t* faceRemap, uint32_t lruCacheSize) {
...
_Use_decl_annotations_ HRESULT DirectX::OptimizeFacesLRU(const uint32_t* indices, size_t nFaces, uint32_t* faceRemap, uint32_t lruCacheSize) {
    if (!indices || !nFaces || !faceRemap)
        return E_INVALIDARG;
...
//-------------------------------------------------------------------------------------
_Use_decl_annotations_ HRESULT DirectX::OptimizeFacesLRUEx(const uint16_t* indices, size_t nFaces, const uint32_t* attributes, uint32_t* faceRemap, uint32_t lruCacheSize) {
    ...
    for (const auto& it : subsets) {
        ...
        HRESULT hr = OptimizeFacesImpl<uint16_t>(&indices[it.first * 3], static_cast<uint32_t>(it.second * 3), &faceRemap[it.first], lruCacheSize, uint32_t(it.first));
        if (FAILED(hr))
            return hr;
    }
...
_Use_decl_annotations_ HRESULT DirectX::OptimizeFacesLRUEx(const uint32_t* indices, size_t nFaces, const uint32_t* attributes, uint32_t* faceRemap, uint32_t lruCacheSize) {
    if (!indices || !nFaces || !attributes || !faceRemap)
        return E_INVALIDARG;
    ...
    for (const auto& it : subsets) {
        ...
        HRESULT hr = OptimizeFacesImpl<uint32_t>(&indices[it.first * 3], static_cast<uint32_t>(it.second * 3), &faceRemap[it.first], lruCacheSize, uint32_t(it.first));
        if (FAILED(hr))
            return hr;
    }
