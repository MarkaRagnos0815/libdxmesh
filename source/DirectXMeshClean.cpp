...
HRESULT CleanImpl(
    ...
    for (uint32_t face = 0; face < nFaces; ++face) {
        ...
        if (i0 == index_t(-1) || i1 == index_t(-1) || i2 == index_t(-1)) {
            ...
            indices[face * 3] = indices[face * 3 + 1] = indices[face * 3 + 2] = index_t(-1);
...
//-------------------------------------------------------------------------------------
_Use_decl_annotations_ HRESULT DirectX::Clean(uint16_t* indices, size_t nFaces, size_t nVerts, uint32_t* adjacency, const uint32_t* attributes, std::vector<uint32_t>& dupVerts, bool breakBowties) {
...
//-------------------------------------------------------------------------------------
_Use_decl_annotations_ HRESULT DirectX::Clean(uint32_t* indices, size_t nFaces, size_t nVerts, uint32_t* adjacency, const uint32_t* attributes, std::vector<uint32_t>& dupVerts, bool breakBowties) {
    HRESULT hr = Validate(indices, nFaces, nVerts, adjacency, VALIDATE_DEFAULT);
    if (FAILED(hr))
        return hr;
