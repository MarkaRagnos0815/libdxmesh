...
class StaticVector {
   public:
    StaticVector() noexcept : m_data{}, m_size(0) {
    }
...
//---------------------------------------------------------------------------------
template <typename T>
float XM_CALLCONV ComputeScore(const InlineMeshlet<T>& meshlet, FXMVECTOR sphere, FXMVECTOR normal, _In_reads_(3) const T* triIndices, _In_reads_(3) const XMFLOAT3* triVerts) noexcept {
...
//-------------------------------------------------------------------------------------
_Use_decl_annotations_ HRESULT DirectX::ComputeMeshlets(
    ...
    return ComputeMeshletsInternal<uint16_t>(indices, nFaces, positions, nVerts, &s, 1u, adjacency, meshlets, uniqueVertexIB, primitiveIndices, &subset, maxVerts, maxPrims);
}
...
_Use_decl_annotations_ HRESULT DirectX::ComputeMeshlets(
    ...
    return ComputeMeshletsInternal<uint32_t>(indices, nFaces, positions, nVerts, &s, 1u, adjacency, meshlets, uniqueVertexIB, primitiveIndices, &subset, maxVerts, maxPrims);
}
...
_Use_decl_annotations_ HRESULT DirectX::ComputeMeshlets(
    ...
    size_t maxVerts,
    size_t maxPrims) {
    return ComputeMeshletsInternal<uint16_t>(indices, nFaces, positions, nVerts, subsets, nSubsets, adjacency, meshlets, uniqueVertexIB, primitiveIndices, meshletSubsets, maxVerts, maxPrims);
}
...
_Use_decl_annotations_ HRESULT DirectX::ComputeMeshlets(
    ...
    size_t maxVerts,
    size_t maxPrims) {
    return ComputeMeshletsInternal<uint32_t>(indices, nFaces, positions, nVerts, subsets, nSubsets, adjacency, meshlets, uniqueVertexIB, primitiveIndices, meshletSubsets, maxVerts, maxPrims);
}

_Use_decl_annotations_ HRESULT DirectX::ComputeCullData(
    const XMFLOAT3* positions,
    size_t nVerts,
    ...
    CullData* cullData,
    MESHLET_FLAGS flags) noexcept {
    return ComputeCullDataInternal<uint16_t>(positions, nVerts, meshlets, nMeshlets, uniqueVertexIndices, nVertIndices, primitiveIndices, nPrimIndices, cullData, flags);
}
...
_Use_decl_annotations_ HRESULT DirectX::ComputeCullData(
    ...
    CullData* cullData,
    MESHLET_FLAGS flags) noexcept {
    return ComputeCullDataInternal<uint32_t>(positions, nVerts, meshlets, nMeshlets, uniqueVertexIndices, nVertIndices, primitiveIndices, nPrimIndices, cullData, flags);
}
