...
bool DirectX::IsValid(const D3D12_INPUT_LAYOUT_DESC& vbDecl) noexcept {
    ...
    for (size_t j = 0; j < vbDecl.NumElements; ++j) {
        ...
        if ((vbDecl.pInputElementDescs[j].AlignedByteOffset != D3D12_APPEND_ALIGNED_ELEMENT) && (vbDecl.pInputElementDescs[j].AlignedByteOffset % alignment) != 0) {
            // Invalid alignment for element
            return false;
        }
...
#if defined(__d3d11_h__) || defined(__d3d11_x_h__)
_Use_decl_annotations_ void DirectX::ComputeInputLayout(const D3D11_INPUT_ELEMENT_DESC* vbDecl, size_t nDecl, uint32_t* offsets, uint32_t* strides) noexcept {
...
#if defined(__d3d12_h__) || defined(__d3d12_x_h__) || defined(__XBOX_D3D12_X__)
_Use_decl_annotations_ void DirectX::ComputeInputLayout(const D3D12_INPUT_LAYOUT_DESC& vbDecl, uint32_t* offsets, uint32_t* strides) noexcept {
...
void ComputeVertexCacheMissRateImpl(_In_reads_(nFaces * 3) const index_t* indices, size_t nFaces, size_t nVerts, size_t cacheSize, float& acmr, float& atvr) noexcept {
    acmr = -1.f;
    atvr = -1.f;
...
//-------------------------------------------------------------------------------------
_Use_decl_annotations_ void DirectX::ComputeVertexCacheMissRate(const uint16_t* indices, size_t nFaces, size_t nVerts, size_t cacheSize, float& acmr, float& atvr) {
    ComputeVertexCacheMissRateImpl<uint16_t>(indices, nFaces, nVerts, cacheSize, acmr, atvr);
}

_Use_decl_annotations_ void DirectX::ComputeVertexCacheMissRate(const uint32_t* indices, size_t nFaces, size_t nVerts, size_t cacheSize, float& acmr, float& atvr) {
